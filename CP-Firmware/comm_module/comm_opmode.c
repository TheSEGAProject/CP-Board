...
/////////////////////////////////////////////////////////////////////////////
uchar ucComm_incLnkBrkCntAndChkForDeadOM2( //RET: 1=Lnk Broken, 0=Lnk still open
    void)
{
	ulong ulTaskID, ulSN;
	uint uiTaskID;
		...
		if (ucComm_waitForMsgOrTimeout(NO_RSSI))
		{
			    ...
			    )))
			{
				...
				if (ucaMSG_BUFF[MSG_IDX_LEN] != MSG_HDR_SZ)
				{
					...
					// Increment the rewceived message count
					ucRXMsgCount++;
...
//
////		ucaMSG_BUFF[MSG_IDX_LEN] = ; 	 // Length
//		ucaMSG_BUFF[ucMsgIndex++] = COMMAND_PKT; // DE ID
//		ucaMSG_BUFF[ucMsgIndex++] = 14;	 // DE Length
//		ucaMSG_BUFF[ucMsgIndex++] = 1;	 // DE Version
//		ucaMSG_BUFF[ucMsgIndex++] = 0x01;	 // Proc ID
//		ucaMSG_BUFF[ucMsgIndex++] = 0x1E;	 // Time XI 0x1EDB0ECE
//		ucaMSG_BUFF[ucMsgIndex++] = 0xFF;	 // Time HI
//		ucaMSG_BUFF[ucMsgIndex++] = 0xFF;	 // Time MD
//		ucaMSG_BUFF[ucMsgIndex++] = 0xFF;	 // Time LO
//		ucaMSG_BUFF[ucMsgIndex++] = 0x03;	 // Trans ID
//		ucaMSG_BUFF[ucMsgIndex++] = 0x5B;	 // Valve state
//		ucaMSG_BUFF[ucMsgIndex++] = 0x00;	 // Duration XI
//		ucaMSG_BUFF[ucMsgIndex++] = 0x00;	 // Duration HI
//		ucaMSG_BUFF[ucMsgIndex++] = 0x00;	 // Duration MD
//		ucaMSG_BUFF[ucMsgIndex] = 0x1E;	 // Duration LO
//
//	}
...
static uchar ucComm_WaitFor_LRQ(void)
{
	...
	uint uiMySN;
	signed int iRSSI;
	uchar ucRSSITaskState;
	ulong ulOtherGuysSN, ulRSSITaskState;
...
//	}
	...
	{
		ucTask_GetField(ucTask_FetchTaskIndex(TASK_ID_RSSI), TSK_STATE, &ulRSSITaskState);
		ucRSSITaskState = (uchar)ulRSSITaskState;

		if (ucRSSITaskState == TASK_STATE_ACTIVE)
		{
...
uchar ucComm_WaitFor_RTR(void)
{
	...
	uint uiMySN;
	signed int iRSSI;
	ulong ulCurrentTime, ulOtherGuysSN, ulRSSITaskState;
...
void vComm_SendAck(void)
{
	...
	// Single message
	vComm_Msg_buildOperational((MSG_FLG_SINGLE | MSG_FLG_ACK), uiMsgNumber, uiOtherGuysSN, MSG_ID_OPERATIONAL);
	...
	while (ucTimeCheckForAlarms(SUBSLOT_ALARMS) == 0)
	{
		if (ucComm_waitForMsgOrTimeout(NO_RSSI))
		{
			    ...
			    )))
			{
				...
				if (ucParseRet == 0x01)
				{
					// Once this function returns exit, either because we are out of time or there are no more messages to send
					vComm_SendData();
					break;
				}
